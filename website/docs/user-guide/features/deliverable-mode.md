...
The agent doesn't reach for artifacts by default — it has to know to. Two
ways to nudge it:

**Per-session:** load the `deliverable-mode` skill at the start of a session
with `/skills load deliverable-mode`. The skill biases the agent toward
producing PDFs, charts, and spreadsheets for comparative or numeric content
when on a messaging gateway.

**Always-on:** add `deliverable-mode` to your auto-loaded skills in
`~/.hermes/config.yaml` so it loads on every gateway session.
